...
export type LoggerOptions = {
  ...
  callSiteOptions?: {
    ...
    /**
     * Custom stack frame offset to control which caller appears in logs.
     *
     * The logger automatically skips its own internal methods to show the actual
     * caller. Use this to adjust how far up the call stack to look.
     *
     * - 0 = The code that called logger.log() / logger.info() etc.
     * - 1 = The parent of the direct caller
     * - 2 = The grandparent of the direct caller
     * - undefined = Auto-detect (default behavior, skips logger internals)
     *
     * @example
     * // In a wrapper function, show the caller of your wrapper instead of the wrapper itself
     * function myWrapper(msg: string) {
     *   logger.info(msg); // With frameIndex: 1, shows who called myWrapper()
     * }
     */
    frameIndex?: number;
  };
